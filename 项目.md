...
{ 
  ...
  if(!res)
  {
    logger_error("result is null:%s", mysql_error(m_mysql), sql.c_str());
    return ERROR;
  }
  //执行sql语句 获取查询结果
  MYSQL_ROW row = mysql_fetch_row(res);
  //获取结果记录
  if(!row) logger_warn("reult is empty: %s, sql: %s", mysql_error(m_mysql), sql.c_str());
  else
  {
    groupname = row[0];
    //结果记录在缓存中备份一份
              ...
              char const* groupname) const
{
  ...
  {
    logger_error("insert databse fail: %s, sql:%s", mysql_error(m_mysql), sql.c_str());
    return ERROR;
  }
  return OK;
}
...
int db_c::get(std::vector<std::string>& groupnames) const
{
  ...
  if(mysql_querey(m_mysql, sql.c_str()))
  {
    logger_error("query database fail: %s, sql: %s",
                 mysql_error(m_mysql), sql.c_str())l
      return ERROR;
  }
  //获取查询结果
  MYSQL_RES* res = mysql_store_result(m_mysql);
...
clss service_c
{
  ...
  int beat(char const* groupname, char const* hostname, 
           char cosnt* saddr) const;
  
  //响应客户机存储服务器地址列表
  int saddrs(acl::socket_stream* conn, char const* appid, 
             char const* userid) const;
  //根据用户ID获取对应组名
  ...
  //应答错误
  bool error(acl::socket_stream* conn, short errnumb， char const* format, ...) const; //变长参数表
};
```
                         ...
                         char const* head) const
{
  ...
  switch(command)
  {
    ...
    default:
      error(conn, -1, "unkown command: %d", command);
      return false;
  }
  return result;
} //一级方法
                     ...
                     long long bodylen ) const
{
  ...
  {
    logger_error("read fail:%s, bodylen: %lld, from: %s", acl::last_serror(),
                 bodylen, conn->get_peer());
    return false;
  }
  //解析包体
  storage_join_t sj;
  ...
  //版本
  strcpy(sj.sj_version, sjb->sjb_groupname);
  //组名
  strcpy(sj.sj.groupname, sjb->sjb_groupname);
  ...
  logger("storage join, version: %s, groupname: %s,"
         ...
         std::string(ctime(&sj.sj_stime))，
         std::string(ctime(&sj.sj_jtime)).s_str());
  //将存储服务器加入组表
                      ...
                      long long bodylen ) const
{
  ...
  storage_beat_body_t* sbb = (storage_join_body_t*) body;
                        ...
                        long long bodylen ) const
{
  ...
  if(conn->read(body, bodylen) < 0)
  {
    logger_error("read fail: %s, bodylen: %lld, form: %s", 
...
{
  //互斥锁加锁
  if ((error = pthread_mutex_lock(&g_mutex)))
  {
    loggererror("call pthread_mutex_lock fail: %s", strerror(errno));
    return false;
  }
  //COUNT OF GROUPS
  //全组字符串
      ...
      )
  {
    ...
    //遍历该组中每一台存储服务器
    for(std::list<storage_infor_t>::const_iterator si = group->second.begin();
        si != group->second.end();
        ++si)
    {
  ...
  {
    loggererror("call pthread_mutex_unlock fail: %s", strerror(errno));
    return false;
  }
  //|包体长度|命令|状态| 组列表 |
  //	 8       1   1   包体长度
...
//将存储服务器加入组表
int  service_c::join(storage_join_t const* sj,
...
//响应客户机存储服务器地址列表
int service_c::saddrs(acl::socket_stream* conn, char const* appid, 
                      char const* userid) const
{
  return OK;
}
//根据用户ID获取对应组名
int service_c::group_of_user(char const* appid, char const* userid,
